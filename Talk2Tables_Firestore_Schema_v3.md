...
|---|---|
| `organizations` | Tenant isolation boundaries |
...
## 0. `organizations/{org_id}`

**Document ID:** UUID

| Field | Type | Description |
|---|---|---|
| `org_id` | string | UUID — same as document ID |
...
| `org_id` | string | Tenant organization ID for strict data isolation |
...
| GET | `/{id}/docs` | db_manager+ | List uploaded business documentation |
| POST | `/{id}/docs` | db_manager+ | Upload new business documentation |
| GET | `/{id}/docs/{doc_id}/download` | db_manager+ | Download raw documentation file |
| DELETE | `/{id}/docs/{doc_id}` | db_manager+ | Delete documentation and rebuild summaries |
...
| v3.6 | 2026-04-06 | • **Multi-Tenant Organization Isolation**: added `organizations` top-level collection <br> • Added `org_id` reference field to `users` and `database_connections` root documents to enforce strict tenant boundaries <br> • Access grants now cross-validate that target user and connection belong to the same `org_id` |
| v3.7 | 2026-04-07 | • **Business Documentation Downloads API**: Added endpoints under `connections/{id}/docs` to list, upload, download, and delete business documentation text. <br> • Enforced multi-tenant isolation via dynamic schema scoping on invalidations. |
