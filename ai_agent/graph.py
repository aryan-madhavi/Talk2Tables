...
async def node_react_agent(state: AgentState) -> AgentState:
    ...
    try:
        ...
        logger.info(f"[node_react_agent] Agent done | length={len(last_ai_content)}")
...
def route_after_output_parser(state: AgentState) -> str:
    if state.get("response_type") == "results": return END
...
def get_agent():
    global _agent_graph
    if _agent_graph is None: _agent_graph = build_agent_graph()
    return _agent_graph
...
_WRITE_BLOCKED_RESPONSE: dict[str, Any] = {
    ...
    "sql_query":           "",
    "summary":             "You don't have permission to modify data. Your role (analyst) only allows SELECT queries.",
    "total_records":       0,
...
async def run_agent(
    ...
    if firebase_uid in _in_flight:
        return {
            ...
            "final_response": {
                "title": "Query in progress", "sql_query": "",
                "summary": "Your previous query is still running. Please wait.",
                "total_records": 0, "numerical_insights": {"total_records": 0, "aggregations": {}}, "data": [],
            },
        }
    ...
    try:
        ...
        initial_state: AgentState = {
            ...
            "db_connection_string":   None, "db_dialect": None, "db_type": None,
            "agent_output": None, "response_type": None, "final_response": None,
...
async def run_agent_stream(
    ...
    if user_role not in _ROLES_ALLOWED_WRITE and _WRITE_INTENT_RE.search(natural_language_query):
        yield f"event: result\ndata: {_json.dumps({'response_type': 'results', 'final_response': _WRITE_BLOCKED_RESPONSE, 'chat_id': chat_id})}\n\n"
        return

    if firebase_uid in _in_flight:
        payload = {"response_type": "results", "final_response": {"title": "Query in progress", "summary": "Waiting..."}, "chat_id": chat_id}
